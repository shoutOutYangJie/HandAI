...
class HandTracker():
    ...
    def detect_hand(self, img_norm, img_small):
        ...
        out_reg = self.interp_palm.get_tensor(self.out_reg_idx)[0]  # (2944, 18)  4+ 2*7
        ...
        # candidate_detect = out_reg[detecion_mask]  # 回归数字  N, 18
        # candidate_anchors = self.anchors[detecion_mask]  # anchor位置,  N, 4
        #
        # if candidate_detect.shape[0] == 0:
        #     # print("No hands found")
        #     return None, None
        # # picking the widest suggestion while NMS is not implemented
        # max_idx = np.argmax(candidate_detect[:, 3])  # 选择height最大的样本
        # # print(self._sigm(out_clf)[detecion_mask][max_idx])

        score = self._sigm(out_clf)
        max_idx = np.argmax(score)
        if score[max_idx] < 0.9:
            return None, None

        candidate_detect = out_reg
        candidate_anchors = self.anchors
        # bounding box offsets, width and height
        dx,dy,w,h = candidate_detect[max_idx, :4]  # (1, 4)
        center_wo_offst = candidate_anchors[max_idx,:2] * 256  #anchor中心的位置 +
        center_wo_offst += 0.35 * np.r_[dx, dy]
        # 7 initial keypoints
        ...
        #     x, y = keypoint
        #     cv2.circle(img_small, (int(x), int(y)), THICKNESS * 2, POINT_COLOR, THICKNESS)
        ...
        source -= (keypoints[0] - keypoints[2]) * self.box_shift  # (3, 2), 3个点， xy坐标
    ...
    def preprocess_img(self, img):
        ...
        return img_pad, img_norm, pad, img_small


    def __call__(self, img):
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # pad 记录了pad的位置，用来从预测结果中取出原图位置
        img_pad, img_norm, pad, img_small = self.preprocess_img(img)

        source, keypoints = self.detect_hand(img_norm, img_small.copy())
        if source is None:
            return None, None
        ...
        # 目的是要让手掌垂直，起到对齐的作用，但送入的原图大小，不是检测到的手掌区域
        img_landmark = cv2.warpAffine(
            self._im_normalize(img_pad), Mtr, (256,256)
        )

        # img_temp =  cv2.warpAffine(
        #     img_pad, Mtr, (256,256)
        # )
        # cv2.imshow('temp', img_temp)

        joints = self.predict_joints(img_landmark)  # (21, 2) 即便是双手也是只能预测21个点
        ...
        box_orig = (self._target_box @ Minv.T)[:,:2]  # (4, 4)
        kp_orig -= pad[::-1]
        box_orig -= pad[::-1]
