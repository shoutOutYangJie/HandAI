...
class CenterFace(object):
    def __init__(self, height, width, landmarks=True, model_path=None):
        self.landmarks = landmarks
        if model_path is None:
            if self.landmarks:
                self.net = cv2.dnn.readNetFromONNX('./ckpts/centerFace/centerface.onnx')
        else:
            if self.landmarks:
    ...
    def __call__(self, img, threshold=0.5):
        ...
        if len(dets) > 0:
            ...
            # select max bounding box.
            width = dets[:, 2] - dets[:, 0]
            height = dets[:, 3] - dets[:, 1]
            area = width * height
            max_mask = np.argmax(area)
            dets = dets[max_mask]
            if self.landmarks:
                lms = lms[max_mask]
        else:
            dets = np.empty(shape=[5], dtype=np.float32)
            if self.landmarks:
                lms = np.empty(shape=[10], dtype=np.float32)
        if self.landmarks:
            return dets, lms
        else:
            return dets
